...
/*
	...
	private void creaGrafo() {
		...
		Graphs.addAllVertices(graph, dao.getAllFermateEnhanced());
		System.out.println(graph);
				
		for(Connessione c : dao.listConnessioniEnhanced()) {
			DefaultWeightedEdge e = graph.addEdge(c.getFermata1(), c.getFermata2());
			if (e != null)
				graph.setEdgeWeight(e, c.getPeso());
		}
	...
	public String getPercorso(FermataEnhanced partenza, FermataEnhanced arrivo) {
		
		DijkstraShortestPath<FermataEnhanced,DefaultWeightedEdge> path = new DijkstraShortestPath<FermataEnhanced,DefaultWeightedEdge>(graph,partenza,arrivo);
	...
	public void edgeTraversed(DefaultWeightedEdge e) {
		...
		FermataEnhanced f1 = graph.getEdgeSource(e) ;
		FermataEnhanced f2 = graph.getEdgeTarget(e) ;
